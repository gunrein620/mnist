...
class NeuralNetwork:
    ...
    def __init__(self, use_batchnorm=True, use_dropout=True, dropout_ratio=0.5):
        ...
        self.params = {}
        self.params["W1"] = np.random.randn(input_size, hidden_size1) * np.sqrt(
            2.0 / input_size
        )
        self.params["b1"] = np.zeros(hidden_size1)
        self.params["W2"] = np.random.randn(hidden_size1, hidden_size2) * np.sqrt(
            2.0 / hidden_size1
        )
        self.params["b2"] = np.zeros(hidden_size2)
        self.params["W3"] = np.random.randn(hidden_size2, output_size) * np.sqrt(
            2.0 / hidden_size2
        )
